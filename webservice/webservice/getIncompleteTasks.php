...
<?php
	if (isset($_POST['staff_id'])){
		...
		$braodcast = array(); 
		//this query returns all tasks a user created and all that were assigned to them that arent completed
		$sql = "SELECT title, due_date, description, id, status_id AS state FROM 
                ...
                FROM task t WHERE t.status_id = 1 AND t.staff_id = '".$userId."') one 
